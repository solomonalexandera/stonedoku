...
- **Managers:** The code in `app.js` is organized into "manager" objects (e.g., `ViewManager`, `PresenceSystem`, `LobbyManager`, `MatchManager`). When adding new functionality, try to fit it within one of these existing managers or create a new one if necessary.
