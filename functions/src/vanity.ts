 ...
 * Routes: /profile/:handle  -> 302 redirect to /profile/<uid> or returns simple OG HTML
 */
export const vanityLookup = onRequest(async (req, res) => {
  try {
    ...
    if (!doc.exists) {
      ...
      res.status(404).send(`<!doctype html><html><head><meta charset="utf-8"><title>Not found</title></head><body>Not found</body></html>`);
      return;
    }
